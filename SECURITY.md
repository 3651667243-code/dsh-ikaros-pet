...
- **TTS 语音桥**：`edge_tts_bridge`（edge-tts 版）与 `vits_server.py`（VITS 版）均为本地 HTTP 服务，默认监听 `127.0.0.1:9880`（仅本机）；WSL 部署时因 Windows 需经 WSL2 localhost 转发访问，`wsl_run_vits.sh` 显式使用 `--host 0.0.0.0`——**建议配置 `--auth-token` 访问令牌**（`VITS_AUTH` 环境变量传入）：配置后 `/tts` 与权重接口需带 `Authorization: Bearer <token>` 或 `?token=<token>`，Sakura 侧在 TTS api_url 里追加 `?token=<令牌>` 即可。无令牌请求返回 401；健康检查 `/` 放行（无副作用）。请仅在可信局域网/本机环境下使用；
- **角色包**：纯配置与立绘素材。
...
- **联系维护者**：`[维护者联系方式占位：GitHub @ikaros-sora，或仓库 README 中的联系方式]`。
