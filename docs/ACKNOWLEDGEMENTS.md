...
## 5. 智谱 GLM-4-Flash / DeepSeek-chat

- **说明**：本项目默认的对话模型是 **DeepSeek-chat**（OpenAI 兼容接口、格式遵循稳定）；也可切换智谱开放平台的 **GLM-4-Flash**（免费、OpenAI 兼容接口、支持 function calling）。感谢两家服务商提供的可负担大模型服务，让桌宠拥有对话与工具调用能力。

## 5.1 Ikaros521/moe-tts（VITS 声线模型）

- **来源**：<https://huggingface.co/Ikaros521/moe-tts>
- **用途**：`tools/edge_tts_bridge/vits_server.py` 的默认语音合成模型（saved_model/19，ikaros 角色声线）。
- **许可证声明**：该模型的**许可证与训练数据来源不明确**，默认视为**不可公开再分发、不可商用**；本项目**不随仓库分发**任何模型权重，使用时请自行到 HuggingFace 确认模型卡片条款与授权边界，并自行承担相应责任。
- **禁止用途**：不得用于冒充真实人物、误导性对话、商业配音、未经授权的广告宣传或任何违法、骚扰、欺诈用途。
...
- **声明**：本项目中的「伊卡洛斯」仅为**角色扮演参考**——借用其「三无、天然呆、绝对忠诚」的角色气质与「主人」「命令吗？」「我会守护主人」「铭刻」等标志性台词风格，向原作致敬。本项目为**非官方个人同人项目**，**不包含《天降之物》的任何官方素材**：立绘为本地个人素材（不入库），仓库仅提供 MIT 可自由使用的占位图生成脚本（`make_placeholder_portraits.py`），欢迎用户用自己的创作替换。若您计划公开传播本项目，请自行确认是否符合当地法律与原作版权方要求；本项目不对任何第三方使用方式负责。
